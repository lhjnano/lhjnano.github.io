...
description: 데이터 웨어하우스(Redshift)와 서버리스 쿼리(Athena), 그리고 파일 스토리지(EFS/FSx)를 정리한다.
...
OLTP에 익숙한 엔지니어가 분석 워크로드(OLAP)를 설계하려면 발상을 전환해야 한다. 행 기반 저장이 아닌 **열 기반 저장**, 프로비저닝이 아닌 서버리스, 블록 스토리지가 아닌 **공유 파일 시스템**이 분석 아키텍처의 핵심이다. 이 글에서는 AWS 분석 서비스(Redshift, Athena)와 파일 스토리지(EFS, FSx)를 정리한다.
...
데이터 웨어하우스(Data Warehouse, DW)는 기업의 다양한 소스에서 수집된 데이터를 **분석 목적**으로 저장하는 중앙 집중식 저장소이다. 트랜잭션 처리(OLTP)가 아닌 **분석 질의(OLAP)** 에 최적화되어 있습니다.

**ETL 파이프라인**이 데이터를 소스에서 추출(Extract)하고, 분석에 적합한 형태로 변환(Transform)한 후, 데이터 웨어하우스에 적재(Load)한다. AWS에서는 AWS Glue가 ETL 엔진 역할을 수행하며, Amazon QuickSight가 BI 도구로 제공된다.

### OLTP vs OLAP

분석 아키텍처를 이해하려면 먼저 두 패러다임의 차이를 명확히 해야 한다.
...
> OLTP는 "한 건을 빠르게" 처리하는 것이 목표이고, OLAP은 "수억 건을 한 번에 집계"하는 것이 목표이다. 목적이 다르면 저장 방식도 달라야 한다.
...
- **컴퓨팅 노드**: 실제 데이터 저장 및 쿼리를 실행합니다. 각 노드는 2개 이상의 Slice로 분할되며, Slice가 쿼리 실행의 최소 병렬 단위가 된다. RA3, DC2, DS2 노드 타입을 제공합니다.
...
평균 나이를 구할 때 age 컬럼만 읽으면 되므로 I/O가 대폭 감소합니다. 동일 타입 데이터가 연속 배치되어 압축률도 높아지며, SUM·AVG·COUNT 등 집계 함수에 최적화된다.
...
분석과 트랜잭션을 같은 데이터베이스에서 처리해도 되지 않을까 생각했는데, OLAP와 OLTP를 분리해야 양쪽 성능이 모두 보호된다는 점을 깨달았습니다. Redshift가 열 단위 저장과 MPP로 페타바이트 분석을 담당하는 구조는, 워크로드의 본질이 다르면 아예 다른 엔진으로 풀어야 한다는 통찰을 줍니다.

가장 큰 충격은 Athena의 과금 모델이었습니다. 서버리스라 편리하다고만 생각했는데, 스캔한 데이터 양만큼 과금한다는 점은 비용 관리를 소홀히 하면 큰 청구서로 이어질 수 있다는 경고였습니다. 하지만 CSV를 Parquet으로 변환하고 파티셔닝을 설계하면 비용을 95%까지 줄일 수 있다는 사실은, 올바른 데이터 포맷 설계가 얼마나 큰 차이를 만드는지 보여줍니다.

파일 스토리지 역시 프로토콜이 정답을 알려준다는 점이 흥미로웠습니다. Linux는 EFS, Windows는 FSx for Windows, HPC는 FSx for Lustre처럼 워크로드 특성에 맞춰 자연스럽게 매핑됩니다. 앞으로 데이터 레이크 아키텍처를 직접 구축하며, 분석과 파일 스토리지가 유기적으로 연결되는 시스템을 설계해 보고 싶습니다.
