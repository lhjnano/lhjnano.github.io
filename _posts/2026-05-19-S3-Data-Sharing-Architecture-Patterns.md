...
처음에는 "버킷 정책 하나면 공유가 되는데 왜 아키텍처 패턴이 필요한가?"라는 의문이 들었습니다. 하지만 조직이 커지면서 계정이 분리되고, 규제가 걸리고, 글로벌 사용자가 붙기 시작하면 "데이터를 어떻게 공유할 것인가"는 더 이상 설정 문제가 아니라 아키텍처 문제가 됩니다. 이 글을 정리하면서 깨달은 것은, 6가지 패턴 각각이 특정한 **조직적 압력**에 대한 응답이라는 점입니다. 데이터 주권이 필요하면 연합 분석이 나오고, 상업 거래가 필요하면 마켓플레이스가 나옵니다.
