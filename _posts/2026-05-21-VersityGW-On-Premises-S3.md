...
온프레미스 환경에서 S3 API를 써야 할 때마다 "결국 AWS에 종속되는 것 아닌가?"라는 막막함이 있었습니다. 하지만 VersityGW를 알게 된 후, 기존 POSIX 파일시스템 위에 S3 API를 얹을 수 있다는 것은 클라우드와 온프레미스 사이의 벽이 생각보다 얇다는 것을 깨달았습니다. 바이너리 하나로 시작해서 로드밸런서 뒤에 여러 인스턴스를 배치하면, 프로덕션 규모의 S3 호환 스토리지를 직접 구축할 수 있습니다.

특히 인상 깊었던 것은 Stateless 아키텍처의 단순함이었습니다. 상태를 저장하지 않기 때문에 수평 확장이 자연스럽고, 공유 파일시스템만 마운트하면 어떤 인스턴스로도 요청을 라우팅할 수 있습니다. 그리고 AWS CLI와 Boto3가 `--endpoint-url` 하나만 추가하면 그대로 작동한다는 점은, S3 프로토콜이 사실상 표준이 된 진짜 이유를 잘 보여줍니다. 이 시리즈 전체를 관통하는 하나의 교훈은, **프로토콜이 곧 자유**라는 것입니다. 표준을 따르면 벤더를 바꿀 수 있고, 클라우드와 온프레미스를 오갈 수 있습니다.

S3 시리즈 7편을 마무리하며 드는 생각은, S3는 결국 "데이터를 어떻게 저장하고, 어떻게 공유하고, 어떻게 보호할 것인가"라는 근본적인 질문에 대한 하나의 모범 답안이라는 것입니다. 이 시리즈가 S3를 단순한 스토리지가 아닌, 데이터 거버넌스와 아키텍처의 관점에서 바라보는 출발점이 되기를 바랍니다.
