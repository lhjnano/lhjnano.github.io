...
IAM을 처음 배울 때는 단순히 "사용자를 만들고 권한을 주는 일"이라고 생각했습니다. 하지만 권한 평가 로직을 깊이 들여다보니 가장 중요한 원칙은 "명시적 Deny가 항상 이긴다"는 사실이었습니다. 어딘가에서 Deny가 하나라도 걸려 있으면 그 요청은 무조건 거부됩니다. 즉, 보안에서는 "무엇을 허락했는가"보다 "무엇을 막았는가"가 더 강력한 통제 수단이라는 점이 인상적이었습니다. 기본이 거부(Default Deny)라는 설계 철학은 최소 권한 원칙을 시스템 수준에서 강제하는 구조이고, 이것이 Access Key를 코드에 박아두는 습관이 왜 위험한지를 근본적으로 설명해 줍니다.

봉투 암호화를 처음 접했을 때는 "왜 키로 키를 또 암호화하는가?"가 의문이었습니다. 그런데 KMS에 모든 데이터를 보내지 않고 로컬에서 암호화를 수행하면서도, 마스터 키는 HSM 안에서만 존재한다는 설계는 성능과 보안을 동시에 잡는 우아한 해법이었습니다. 이는 결국 "신뢰의 범위를 좁히고, 검증 가능한 흔적을 남기는" 일이 보안의 본질이라는 통찰로 이어집니다. CloudHSM과 XKS까지 고려하면, 규제 환경에 따라 "어디까지 제어권을 가져갈 것인가"를 선택할 수 있다는 점이 클라우드 보안의 유연성을 보여줍니다.

보안은 완성되는 것이 아니라 지속적으로 다듬어가는 설계 과정이라고 생각합니다. Access Analyzer로 미사용 권한을 찾아내고, Identity Center로 접근을 중앙화하고, 키 교체를 자동화하는 일이 곧 "보안 부채"를 갚는 과정입니다. IAM과 KMS는 단순한 도구가 아니라, 클라우드에서 신뢰를 설계하는 사고방식의 출발점이라는 점을 앞으로의 아키텍처 설계에 계속 적용하고 싶습니다.
