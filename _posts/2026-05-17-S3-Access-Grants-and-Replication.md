---
layout: post
title: "[S3 3/7] S3 Access Grants와 복제로 대규모 데이터 공유하기"
categories: [AWS, S3]
...
IAM 정책으로 권한을 관리하다 보면, 어느 순간 JSON 파일이 수백 줄로 불어나면서 "이 정책이 정확히 누구에게 무엇을 허용하는지" 아무도 확신할 수 없는 상태가 됩니다. Access Grants를 처음 봤을 때, 이것은 단순히 IAM 정책을 대체하는 도구가 아니라 권한 관리의 **발상 전환**이라는 느낌을 받았습니다. "정책을 쓴다"가 아니라 "권한을 부여한다"는 패러다임 이동이었고, 디렉토리 그룹과 연동된다는 점은 조직이 성장해도 권한 관리가 선형적으로 유지될 수 있음을 의미했습니다.

복제를 공부하면서 다시 한번 느낀 것은, "데이터를 공유한다"는 행위에 **물리적 복사**와 **논리적 위임**이라는 두 가지 축이 존재한다는 사실입니다. Access Points나 Access Grants는 원본을 그대로 두고 접근만 위임하지만, CRR과 SRR은 데이터를 물리적으로 복사합니다. 처음에는 복제가 비용 낭비처럼 보였지만, 데이터 주권과 재해 복구, 지연 시간 최소화라는 요구사항을 마주하고 나면 복제가 유일한 해법인 순간이 반드시 찾아옵니다.

이 글을 읽고 계신 분이 대규모 조직에서 권한 관리에 지쳐 있다면, Access Grants로 정책 폭발에서 벗어나는 것을 먼저 시도해 보시길 바랍니다. 그리고 글로벌 팀과 일한다면 복제 전략을 "비용"이 아니라 "사용자 경험"의 관점에서 다시 설계해 보세요.
