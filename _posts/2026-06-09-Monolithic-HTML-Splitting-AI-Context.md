...
파일 하나가 346KB, 7,600줄이 되었을 때 처음 한 생각은 "단일 파일 PWA니까 어쩔 수 없지"였습니다. 단일 파일은 오프라인 보장과 무료 호스팅이라는 장점이 있으니, 파일 크기가 큰 것은 그 아키텍처의 대가라고 받아들였습니다. 하지만 이것은 의도적 단일 파일과 비의도적 모놀리식을 혼동한 것이었습니다. 빌드 도구 없이도 파일을 분할할 수 있다는 사실을 깨닫기까지 꽤 오래 걸렸고, 분할 후에도 원래 아키텍처의 이점을 그대로 유지할 수 있다는 것을 확인했을 때 비로소 "아, 파일 크기와 아키텍처는 별개의 문제였구나"라고 말할 수 있었습니다.

분할 작업에서 가장 의미 있었던 발견은 파일이 작아졌을 때 가장 큰 수혜자가 사람이 아니라 **AI 에이전트 자체**라는 점입니다. 346KB 파일을 매번 통째로 읽던 AI는 컨텍스트를 거의 다 소진해버려 수정 품질이 떨어질 수밖에 없었습니다. 반면 200줄짜리 파일만 읽는 AI는 컨텍스트에 여유가 남고, 그 여유가 곧 출력 품질로 직결됩니다. "AI 컨텍스트 절약"을 파일 분할의 1순위 동기로 삼게 된 결정적인 이유였습니다.

점진적 마이그레이션을 선택한 것도 현명한 판단이었습니다. 한 번에 전체를 재작성하면 반드시 무언가 망가지지만, "다른 이유로 파일을 수정할 때 그 부분만 추출한다"는 전략은 느려 보이면서도 재작업이 없어서 결과적으로 더 빠릅니다. AI가 생성하는 코드량이 앞으로 더 늘어날수록, AI가 효율적으로 읽고 수정할 수 있는 파일 구조를 유지하는 일은 단순한 정리가 아니라 개발 생산성의 핵심이 될 것입니다.
