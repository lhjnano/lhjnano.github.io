...
S3를 처음 다룰 때 가장 놀랐던 점은 "폴더가 사실은 존재하지 않는다"는 사실이었습니다. `aws s3 ls`를 치면 마치 디렉토리처럼 보이지만, 그 이면은 평면적인 Key-Value 구조일 뿐입니다. 이 단순한 사실을 이해하고 나면, S3의 설계 철학 전체가 달라 보입니다. 파일 시스템의 복잡한 계층 구조를 버리고 오직 HTTP와 평면 네임스페이스만 남겼기 때문에, S3는 무한히 확장할 수 있고 어떤 환경에서든 동일한 방식으로 접근할 수 있습니다.

멀티파트 업로드와 Pre-signed URL을 배우면서 깨달은 것은, S3의 진짜 강점이 저장 용량이 아니라 **접근 방식의 유연성**이라는 점입니다. 5TB짜리 파일을 파트별로 병렬 업로드하고, 실패한 조각만 다시 보낼 수 있다는 것은 곧 "대용량 처리 = 인프라 문제가 아니라 패턴 문제"라는 뜻이었습니다. IAM 자격 증명 없이 URL 하나로 권한을 위임하는 Pre-signed URL도 마찬가지로, 보안과 편의성 사이의 균형을 프로토콜 수준에서 풀어낸 훌륭한 설계입니다.

S3 API가 이렇게 널리 표준이 된 이유는 결국 **단순함**에 있습니다. 6개의 HTTP 메서드로 모든 것을 표현하는 RESTful 설계 덕분에 MinIO, Cloudflare R2, VersityGW 같은 호환 생태계가 자연스럽게 성장했습니다. 앞으로 S3를 다루신다면, API 스펙만 익히는 것에 그치지 말고 이 단순함이 만들어낸 생태계 전체를 하나의 언어로 바라보시길 바랍니다.
