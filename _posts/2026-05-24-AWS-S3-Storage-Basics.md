...
description: S3의 기본 개념부터 스토리지 클래스, 버전 관리, 수명주기 규칙, 암호화, 복제, 성능 최적화까지 기초를 정리한다.
...
S3는 "무제한으로 저장하고, 언제 어디서나 가져오는" 객체 스토리지이다. 99.999999999%(11개의 9) 내구성, REST API 기반 접근, TB 단위 파일까지 한 번에 처리한다. 하지만 버킷 이름 하나 잘못 짓거나 암호화를 켜지 않으면 하루 만에 문제가 시작됩니다.

이 글에서는 S3의 기초를 처음부터 끝까지 정리한다. 버킷과 객체의 구조, 7가지 스토리지 클래스, 보안/암호화 기본, 버전 관리, Lifecycle 규칙, 복제, 이벤트 알림, 성능 최적화까지 한 흐름으로 살펴봅니다.

---

S3는 "그냥 저장소"라고 생각하기 쉬운데, 스토리지 클래스만 7종이고 수명주기 규칙까지 들어가면
의외로 결정할 게 많다. 이 글은 그 결정을 단계적으로 정리한다.
...
버킷은 객체를 저장하는 기본 컨테이너입니다. 버킷 이름은 모든 AWS 계정, 모든 리전에서 유일해야 한다.
...
**멀티파트 업로드**는 대용량 파일을 여러 파트로 나누어 업로드하는 기능입니다. 100MB 이상 파일에 권장하며, 5GB 초과 파일은 필수입니다. 파트 크기는 5MB~5GB, 최대 10,000개 파트까지 가능한다. 실패한 파트만 재전송할 수 있어 네트워크 장애에 강한다.
...
S3는 단순한 파일 저장소라고만 생각했는데, 객체 스토리지의 핵심이 Key-Value 구조라는 점을 이해하고 나서 보는 시선이 완전히 달라졌습니다. 평면적인 주소 공간과 REST API로 접근한다는 것은 곧 무한히 확장 가능한 데이터 공간을 의미했고, 이 구조를 잡고 나니 스토리지 클래스와 Lifecycle, 보안까지 모두 자연스럽게 연결되었습니다.

특히 인상 깊었던 것은 비용 최적화의 자동화입니다. 데이터 액세스 패턴에 맞춰 Standard, IA, Glacier를 수동으로 옮길 필요 없이 Lifecycle 규칙 하나로 자동 전환된다는 점은 운영 부담을 극적으로 줄여줍니다. 보안 역시 IAM 정책, 버킷 정책, Block Public Access, 암호화를 겹겹이 쌓는 다층 방어 설계가 필요하다는 걸 배웠습니다.

객체 스토리지는 데이터 레이크, 정적 호스팅, 백업까지 현대 클라우드 아키텍처의 기반이 되는 서비스입니다. 앞으로 S3를 직접 다뤄보며 버전 관리, 복제, 액세스 제어까지 안전한 스토리지 환경을 완성해 나가고 싶습니다.
