...
버킷을 하나 공개했다가 로그에 수천 건의 무단 접근 시도가 찍히는 것을 본 적이 있습니다. 그 순간 "보안은 설정이 아니라 설계다"라는 말이 몸으로 와닿았습니다. IAM 정책 하나를 잘 쓴다고 안전해지는 것이 아니라, 네트워크, 암호화, 접근 제어, 감사가 각각 독립된 계층으로 쌓여 있어야 한 계층이 무너져도 전체가 무너지지 않는다는 것을 깨달았습니다.

처음에는 7계층이라는 숫자가 과하지 않나 싶었습니다. 하지만 실제 사고를 분석해 보면, 뚫린 것은 항상 한 계층이었고 그다음 계층만 있었어도 막을 수 있는 일이었습니다. Block Public Access를 계정 수준에서 켜는 것은 단순한 설정 하나가 아니라 "사람은 실수한다"는 전제를 시스템 수준에서 방어하는 일입니다. SSE-KMS로 암호화 키의 통제권을 갖고, CloudTrail로 모든 접근을 추적하고, Macie로 민감 데이터를 자동 탐지하는 것까지 갖추면 비로소 사고 후에도 "무슨 일이 있었는지" 말할 수 있는 상태가 됩니다.

보안은 완성되는 것이 아니라 지속적으로 점검하는 과정입니다. 이 글의 체크리스트를 한 번이라도 스캔해 보시고, 아직 켜지 않은 계층이 있다면 오늘 당장 하나씩 시작해 보시길 바랍니다.
