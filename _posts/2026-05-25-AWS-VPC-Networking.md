...
네트워크는 클라우드에서 가장 추상적이고 다루기 어려운 영역이라고만 생각했는데, VPC를 CIDR 설계부터 차근차근 따라가 보니 모든 것이 논리적으로 연결된다는 걸 깨달았습니다. IP 대역을 한 번 잘못 잡으면 온프레미스나 다른 VPC와 충돌해 복구가 어렵다는 점은, 초기 설계의 무게를 실감하게 합니다.

보안 설계에서도 새로운 통찰을 얻었습니다. Security Group과 NACL을 단순히 "방화벽"이라고만 생각했는데, Stateful과 Stateless, 인스턴스 단위와 서브넷 단위라는 차이가 각자 다른 역할을 부여한다는 걸 알게 되었습니다. 그리고 연결 규모에 따라 Peering과 Transit Gateway를 선택하는 기준이 명확해지니, 네트워크 아키텍처가 체계적으로 그려지기 시작했습니다.

VPC는 클라우드 인프라 전체의 뼈대이자 보안의 시작점입니다. 앞으로 실제 환경에서 다중 AZ 서브넷을 구성하고, NAT Gateway와 엔드포인트로 비용까지 최적화한 견고한 네트워크를 직접 설계해 보고 싶습니다.
