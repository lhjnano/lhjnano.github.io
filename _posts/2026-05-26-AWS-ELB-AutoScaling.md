...
로드밸런서는 트래픽을 나눠주는 단순한 역할만 한다고 생각했는데, ALB·NLB·GWLB가 각각 프로토콜과 성능 요구사항에 따라 완전히 다른 메커니즘으로 동작한다는 걸 알게 되었습니다. 특히 NLB가 초저지연과 고정 IP, Client IP 보존을 위해 L4에서 동작한다는 점은, 요구사항에 따라 기술의 층위 자체가 달라진다는 통찰을 주었습니다.

Auto Scaling도 인상 깊었습니다. 단순히 "서버를 늘리고 줄이는" 기능이라고 생각했는데, Launch Template으로 버전 관리를 하고 Target Tracking으로 안정적으로 용량을 조정하며, ELB 상태 검사 기반으로 비정상 인스턴스를 자동 교체한다는 것은 사실상 무중단 자동 복구 시스템이었습니다. CLB가 2026년 Deprecated되어 ALB/NLB로 마이그레이션해야 한다는 점도 시대의 변화를 느끼게 합니다.

트래픽 패턴을 예측하고 대응하는 것은 운영 우수성의 핵심입니다. 앞으로 예약 기반 스케일링과 Predictive Scaling을 직접 구성해 보며, 고가용성과 비용 최적화를 동시에 달성하는 아키텍처를 경험해 보고 싶습니다.
