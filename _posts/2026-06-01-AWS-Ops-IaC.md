...
운영 자동화를 처음 접했을 때는 "CloudWatch 대시보드를 보는 일"이 모니터링의 전부라고 생각했습니다. 하지만 CloudWatch, CloudTrail+Config, CloudFormation/CDK를 함께 놓고 보니 이 셋이 답하는 질문이 근본적으로 다르다는 것을 깨달았습니다. "지금 시스템이 정상인가"(관측), "누가 무엇을 바꿨는가"(감사), "이 인프라를 다시 만들 수 있는가"(재현) — 세 질문이 모두 답답함 없이 해결되어야 비로소 운영이 성립한다는 점이 새로웠습니다. 인프라를 코드로 관리한다는 것은 단순히 편리함을 넘어, "재현 가능성"이라는 신뢰의 기반을 다지는 일이라는 생각이 들었습니다.

다중 계정 환경에서 Organization Trail과 Config Aggregator, StackSets를 배치하는 패턴을 보면서, "중앙 집중"이 왜 정답인지 체감했습니다. 각 계정이 독립적으로 로그를 관리하고 규칙을 배포하면 필연적으로 사각지대가 생깁니다. 하지만 보안 전용 Audit 계정에서 모든 계정의 API 호출과 규정 준수 상태를 한 곳에서 집중하면, "보이지 않는 위험"을 "보이는 지표"로 바꿀 수 있습니다. 이는 결국 분산된 환경에서 통제력을 잃지 않는 유일한 방법이라는 생각이 들었습니다.

가장 인상적이었던 것은 자동화의 사각지대를 없애는 파이프라인 구성입니다. CloudWatch 경보가 EventBridge를 타고 Lambda/SSM으로 자동 복구되고, Config 위반이 Remediation을 통해 자동 수정되며, CDK Pipelines와 cdk-nag가 배포 전에 보안 규칙을 검증하는 구조는 "사람의 개입 없이도 안정성이 유지되는" 순환 구조를 만듭니다. 운영 자동화는 단순히 반복 작업을 줄이는 것이 아니라, 시스템이 스스로를 교정하는 복원력을 설계하는 일이라고 생각합니다. 앞으로는 모든 운영 결정이 "재현 가능하고, 추적 가능하고, 자동으로 교정되는가"라는 질문 아래에서 이루어지도록 설계하고 싶습니다.
