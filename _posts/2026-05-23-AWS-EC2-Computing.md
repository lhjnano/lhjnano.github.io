...
description: EC2 인스턴스 유형, 구매 옵션, EBS 볼륨, 배치 그룹, Elastic IP, 수명주기까지 EC2 컴퓨팅의 모든 것을 정리한다.
...
> 클라우드에서 가상 서버 한 대를 띄우는 데 고려할 것이 왜 이렇게 많을까요? 인스턴스 유형만 수십 종, 구매 옵션 네 가지, 스토리지도 영구와 임시로 나뉩니다. 그럼에도 EC2는 AWS 컴퓨팅의 심장이다.

EC2는 필요한 만큼 가상 서버를 프로비저닝하고 보안·네트워크·스토리지를 관리하는 서비스이다. 이 글에서는 EC2의 핵심 구성 요소. 인스턴스 유형, 구매 옵션, EBS, AMI, 배치 그룹, 수명주기, RAID. 를 2026년 최신 기준으로 정리한다.

---

EC2는 AWS의 시작점이지만, 막상 선택하려면 종류가 너무 많아서 당황한다.
이 글은 그 선택을 줄여주는 가이드다.
...
EC2는 VPC 내 서브넷에 인스턴스를 프로비저닝하고, Internet Gateway를 통해 외부 통신을 수행한다.
...
EC2 인스턴스는 SSH(Linux) 또는 RDP(Windows)로 원격 접속한다. 운영체제별 기본 사용자 이름이 다릅니다.
...
인스턴스 스토어는 버퍼·캐시·임시 데이터(scratch)에 적합하며, 데이터베이스나 영구 저장이 필요한 데이터에는 부적합합니다. 중지 불가. 중지하면 데이터가 손실된다.
...
#실무에서 가장 많이 보는 조합은 t3(개발) → m5(운영) → c5(연산 집중) 순이다.
나머지는 특수 목적이라 필요할 때 찾아도 늦지 않다.
