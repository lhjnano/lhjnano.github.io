...
처음 Cognito를 공부할 때는 User Pool과 Identity Pool을 같은 것으로 헷갈렸습니다. "둘 다 인증 아닌가?"라고 생각했는데, 실제로는 User Pool이 "당신은 누구인가"를, Identity Pool이 "당신은 무엇을 할 수 있는가"를 담당한다는 점에서 명확히 역할이 분리되어 있었습니다. JWT를 발급받아 STS 임시 자격 증명으로 변환하는 결합 패턴을 이해하고 나서야, 인증(Authentication)과 권한 부여(Authorization)가 어떻게 연결되는지 체감할 수 있었습니다. 백엔드 인프라 없이 소셜 로그인과 SAML SSO까지 구성할 수 있다는 사실은 클라우드가 가져온 추상화의 깊이를 실감하게 합니다.

보안 서비스들을 나열하다 보면 "도구가 너무 많다"는 느낌이 들었습니다. 하지만 Shield, WAF, Network Firewall, IAM/KMS, GuardDuty를 Edge → Network → Application → Monitoring 계층으로 정렬하는 순간, Defense in Depth의 설계 의도가 선명해졌습니다. 한 계층이 뚫려도 다음 계층이 막아주는 구조는, 결국 "신뢰를 분산시키고 실패를 국소화하는" 엔지니어링 원칙의 보안 버전이라는 생각이 들었습니다. ACM이 인증서 발급·갱신을 완전히 자동화하는 것도 같은 맥락에서, "반복되는 운영 위험을 시스템에 위임하는" 일이라고 볼 수 있습니다.

방어를 겹겹이 쌓는 일은 끝이 없는 과정이라고 생각합니다. 오늘의 완벽한 구성도 새로운 공격 벡터 앞에서는 구멍이 될 수 있기 때문입니다. Firewall Manager로 다중 계정 정책을 중앙에서 관리하고, GuardDuty와 Security Hub로 위협을 지속 탐지하는 것이 곧 "보안은 상태가 아니라 관행"이라는 말의 실천 방식이라고 느꼈습니다. 앞으로도 각 계층의 역할을 명확히 이해하고, 한 계층의 실패가 전체 시스템의 실패로 이어지지 않도록 설계하는 사고를 유지하고 싶습니다.
