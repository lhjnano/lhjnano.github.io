...
1. **엔드포인트 경로 하나가 4일을 날렸다** — `/api/paas/`가 아니라 `/api/coding/paas/`를 사용해야 합니다. 에러 메시지가 "잔액 부족"이라 오해하기 쉽습니다.
2. **JSON 파싱은 4단계 방어** — 직접 파싱 → 코드블록 추출 → 제어문자 수정 → trailing comma 제거. `split('\n').join('\\n')`은 절대 사용하면 안 됩니다.
3. **max_tokens는 필수** — thinking 모델은 설정하지 않으면 응답이 중간에 잘립니다. 4096 이상 설정합니다.
