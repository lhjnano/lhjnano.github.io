---
layout: post
title: "데이터베이스 & 캐시 — RDS, Aurora, ElastiCache"
...
1. **Multi-AZ는 DR, Read Replica는 성능** — 두 복제 방식의 목적을 구분해서 설계해야 합니다. 프로덕션에서는 Multi-AZ + Read Replica 조합이 표준이며, Aurora는 분산 스토리지로 이를 하나로 통합합니다
2. **Redis가 Memcached보다 사실상 표준** — 복제, 장애조치, 영속성, 다양한 데이터 구조를 모두 갖춘 Redis가 대부분의 캐싱 시나리오에 적합합니다. Memcached는 단순 일시적 캐시에만 고려합니다
3. **Lazy Loading + TTL이 캐싱의 정석** — 실제 사용되는 데이터만 캐싱하여 메모리를 효율적으로 쓰고, TTL로 데이터 신선도를 보장하는 조합이 가장 검증된 패턴입니다
