...
window.addEventListener('load', function() {
  ...
  // ---- Chart 1: BLAKE3 SIMD Scaling (4M block) ----
  new Chart(document.getElementById('chart-blake3-simd'), {
    type: 'bar',
    data: {
      labels: ['generic\n(스칼라)', 'SSE2', 'SSE4.1', 'AVX2'],
      datasets: [{
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: 'BLAKE3 SIMD 가속 효과 — generic 대비 AVX2가 15.9배 빠름',
        ...
        tooltip: {
          callbacks: {
            label: function(ctx) {
              ...
              const speedup = (v / 166).toFixed(1);
              return v + ' MB/s (' + speedup + 'x)';
            }
          }
        }
      },
      scales: {
        y: {
          beginAtZero: true,
          title: { display: true, text: '처리량 (MB/s)', color: TICK_COLOR, font: { family: FONT_FAMILY }},
          ticks: { color: TICK_COLOR, font: { family: FONT_FAMILY }},
          grid: { color: GRID_COLOR }
        },
        x: {
          ticks: { color: '#c9d1d9', font: { size: 13, family: FONT_FAMILY }},
          grid: { display: false }
        }
      }
    }
  ...
  // ---- Chart 2: LZ4 restrict A/B Comparison ----
  new Chart(document.getElementById('chart-lz4-ab'), {
    type: 'line',
    data: {
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: 'LZ4 restrict A/B 벤치마크 — 두 선이 겹침 = 성능 차이 없음',
          font: { size: 14, family: FONT_FAMILY },
          color: '#c9d1d9',
          padding: { bottom: 16 }
        },
        legend: {
          labels: { color: '#c9d1d9', font: { family: FONT_FAMILY, size: 13 }}
        }
      },
      scales: {
        y: {
        ...
        x: {
          title: { display: true, text: '블록 크기', color: TICK_COLOR, font: { family: FONT_FAMILY }},
          ticks: { color: '#c9d1d9', font: { family: FONT_FAMILY }},
          grid: { color: GRID_COLOR }
        }
      }
    }
  ...
  // ---- Chart 3: Compression Throughput ----
  new Chart(document.getElementById('chart-compression'), {
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: '압축 알고리즘별 해제 처리량 (1GB 압축 가능 데이터)',
          font: { size: 14, family: FONT_FAMILY },
          color: '#c9d1d9',
          padding: { bottom: 16 }
        },
        tooltip: {
          callbacks: {
            label: function(ctx) { return ctx.parsed.x + ' GB/s'; }
          }
        }
      },
      scales: {
        x: {
          beginAtZero: true,
          title: { display: true, text: '처리량 (GB/s)', color: TICK_COLOR, font: { family: FONT_FAMILY }},
          ticks: { color: TICK_COLOR, font: { family: FONT_FAMILY }},
          grid: { color: GRID_COLOR }
        },
        y: {
