...
<details markdown="1">
<summary>Storage Gateway 개요</summary>
...
<details markdown="1">
<summary>AWS DataSync — 온라인 마이그레이션</summary>
...
<details markdown="1">
<summary>하이브리드 마이그레이션 (온라인 + 오프라인 조합)</summary>
...
클라우드 마이그레이션이라고 하면 항상 "네트워크로 데이터를 전송하는 일"만 떠올렸습니다. 그래서 1Gbps 회선으로 100PB를 옮기는 데 30년이 걸린다는 사실을 처음 봤을 때 큰 충격을 받았습니다. 하지만 Snow Mobile이라는 "트럭"이 수 주 안에 같은 양을 옮긴다는 것을 알고 나니, 때로는 물리적 세계가 디지털 세계보다 빠를 수 있다는 역설이 묘하게 설득력 있게 다가왔습니다. 마이그레이션의 본질은 네트워크가 아니라 "데이터 규모와 시간 제약에 가장 효율적인 수단을 선택하는 일"이라는 것을 이 단계에서 처음 체감했습니다.

Storage Gateway의 네 가지 유형을 살펴보면서 "프로토콜을 보존한다"는 개념이 얼마나 강력한지 깨달았습니다. NFS, SMB, iSCSI, VTL이라는 기존 프로토콜을 그대로 두면서 백엔드만 S3나 Glacier로 바꾸면, 애플리케이션 코드를 한 줄도 수정하지 않고도 클라우드 스토리지의 이점을 누릴 수 있습니다. 이는 "점진적 마이그레이션"의 전제 조건이며, 모든 것을 한 번에 바꾸는 빅뱅(big-bang) 전략이 위험한 이유를 설명해 줍니다. DataSync가 증분 전송과 체크섬 검증으로 온라인 이관을 안정화하는 것도 같은 맥락에서 "신뢰를 단계적으로 구축하는" 설계라는 생각이 들었습니다.

가장 인상적이었던 통찰은 온라인과 오프라인을 "조합"하는 하이브리드 전략입니다. Cold 데이터는 Snow로 오프라인 이관하고, Hot 데이터는 DataSync로 지속 동기화하며, 데이터베이스는 DMS CDC로 컷오버 다운타임을 30분 미만으로 줄이는 설계는 각 도구의 장점을 극적으로 결합한 결과입니다. 마이그레이션은 단발성 이벤트가 아니라 "평가 → 이관 → 최적화"의 지속적 사이클이라고 생각합니다. 앞으로는 어떤 시스템을 이관하든 "네트워크가 정답이라는 편견"을 버리고, 데이터 성격과 규모에 따라 가장 적합한 수단을 조합하는 사고를 유지하고 싶습니다.
