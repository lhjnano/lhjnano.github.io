...
이 원칙은 **[다음 글(2)]({% post_url 2026-06-21-Design-Gate-4-Stage-Preview-Control %})**에서 4단계 게이트와 함께 상세히 다룹니다.
...
` t('ui.a_065')` 같은 구조적 키의 가독성은 최악입니다. 대신 `t('운동이 저장되었습니다')`처럼 **원문 자체를 키로 사용**하면 키 발명 단계가 사라집니다. 이 방식은 **[글(4)]({% post_url 2026-06-23-Gettext-Original-Key-i18n %})**에서 상세히 다룹니다.

### 원칙 7: 컴포넌트 격리 우선, E2E 최후

> 바텀업 테스트. 느리고 불안정한 E2E는 스모크 3~10개만.

전통적 E2E 우선 접근은 CI를 차단합니다. Unit → Component → Contract → E2E Smoke 순으로, **가장 빠르고 안정적인 테스트를 최우선**으로. 이 전략은 **[글(5)]({% post_url 2026-06-24-Layered-Testing-E2E-Last %})**에서 다룹니다.

### 원칙 8: 규칙은 기계가 지킨다

> 사람은 판단에만 쓰고, 규칙 검증은 기계에 위임합니다.

pre-commit → CI 게이트 → workflow 검증 기준으로 **3중 강제** 구조를 만들었습니다. "데드 버튼을 만들지 마세요"라는 팀원 간 약속보다, CI가 `onclick=""` 패턴을 감지해서 PR을 차단하는 것이 확실합니다. **[글(6)]({% post_url 2026-06-25-CI-Dead-Button-Detection-3-Tier-Gate %})**에서 상세히 다룹니다.
...
|---|---|---|
| — | **(1) 10가지 함정** | **[(2) 4단계 디자인 게이트]({% post_url 2026-06-21-Design-Gate-4-Stage-Preview-Control %})** |
